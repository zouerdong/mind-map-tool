...
it("A+ after B must increase the effective default size and preserve bold", () => {
  ...
  expect(onCommand.mock.calls[0]![0].runs).toEqual([
    { start: 0, end: 4, bold: true, fontSize: 18 },
  ]);
});
it("WenKai requested bold must reach the existing fauxBold scene path", () => {
  ...
  const result = buildScene(
    doc,
    { regular: () => ({ advance: () => 10, ascentRatio: 0.8 }), bold: () => null },
    () => "LXGW WenKai",
  );
...
it("a recovered newer ready intent must not be overwritten by the older failed intent on flush", async () => {
  ...
  const barrier = new GeometryBarrier({
    ...
    whenMetricsReady: async () => fonts,
    getFallbackFonts: () => {
      if (fail) throw new Error("transient metrics failure");
      return fonts;
    },
    onError: () => {},
  });
...
it("whole-node B must style unformatted gaps while retaining existing fontSize", () => {
  ...
  doc.document.nodes.push({
    ...
    position: { x: 0, y: 0 },
    size: { width: 120, height: 50 },
    runs: [{ start: 1, end: 2, fontSize: 18 }],
  });
  const onCommand = vi.fn();
  render(
    <ContextToolbar
  ...
  for (let i = 0; i < 4; i++)
    expect(
      command.runs.some(
        (r: { start: number; end: number; bold?: boolean }) => r.start <= i && r.end > i && r.bold,
      ),
    ).toBe(true);
  expect(
    command.runs.find((r: { start: number; end: number }) => r.start <= 1 && r.end > 1).fontSize,
  ).toBe(18);
});
