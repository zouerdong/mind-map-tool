...
- **已否决项（2026-09-18 负责人定稿，不再翻案）**：Windows 键鼠「左键拖空白 = 平移」——
  左键是内容操作（框选）的唯一入口，平移归滚轮/中键/Space+拖；不做平台手势分叉，
  不做偏好开关。依据：恢复左键平移会让框选退回 Shift+拖的不可发现状态（本批次修复
  的原投诉），且滚轮平移在鼠标上已是最顺手入口。
