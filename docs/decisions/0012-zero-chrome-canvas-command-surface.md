...
- Status: Accepted
- ADR-Version: 1.2.0
- Date: 2026-09-08（v1.1.0 修订 2026-09-13；v1.2.0 修订 2026-09-14；v1.3.0/v1.3.1 修订 2026-09-15）
...
3c. **[v1.3.1] 整理/还原双向开关**（OFR-2026-09-15，[from-user] 2026-09-15 负责人实机定稿）：「整理」不再是单向命令——整理后再次触发（浮动按钮或 ⇧⌘L，同一 dispatcher）即把布局拨回整理前快照；快照为 session 级（不入文档文件，与 viewport 同红线），仅拨回仍存在节点的位置（其间新增不动、删除丢弃），文档被替换时快照自动失效退化为正常整理。还原以正向 MoveNodes 进历史（自身可撤销），整理后的手动位移被覆盖但可经 ⌘Z 找回——不丢任何中间编辑。浮动按钮文案随态切换（整理 ⇧⌘L ↔ 还原布局 ⇧⌘L）。
