...
- v1.1.0（2026-09-17，同日 dogfood 决定）：horizontal 默认改为**平衡双侧布局**——organize 单侧层叠遇"宽而浅"树会排成一长列竖带（负责人："读着累"），纵向则排成 9400px 长横带，两个极端都不可读。平衡双侧（根居中、分支按子树高度贪心均分两侧、父在子列内垂直居中）是宽而浅树的可读默认；非单根树（多根/孤儿/交叉多父/环/坐标超限）自动回退 organize 单侧层叠。实现位于 packages/headless（纯几何布局，不改 core、不改 GUI 整理契约）；balanced 参数可关。
...
3a. （v1.1.0）布局默认：direction=horizontal 时默认应用平衡双侧布局（balanced 参数默认 true）；vertical 或守卫回退时使用 organize 单侧层叠。平衡布局在 headless 内实现，坐标上限守卫失败即整体回退，不产非法坐标。
