...
- Status: **Accepted**（G-SCHEMA 批准，2026-09-06 项目负责人；v1.1.0 修订 2026-09-15 负责人）
- Date: 2026-09-06（v1.1.0 修订 2026-09-15：首节点自动强调的例外条款，见 §1 emphasis 行）
...
- `emphasis?: boolean` —— 强调角色；缺省 `false`；不自动派生。**[v1.1.0 例外，OFR-2026-09-15 [from-user] 负责人定稿]**：空文档创建的**第一个节点**自动 `emphasis: true`（「出发点」橙卡——「不管到什么时候，都记得我们的出发点在哪里」）；判定含 pending 创建（并发/待提交不重复标记）；此后创建不派生，用户可经上下文工具条「强调」自由开关；删光全部节点后重建的第一个节点再次成为出发点（规则锚定「空文档」而非历史首次，零 schema 负担）。既有文档不做追溯改写。
