...
当前整改批次状态为 `IN_PROGRESS / R2_REVISE / R2-F1_STAGE_A_READY / PRR-070_BLOCKED`。R2 正常预检记录保留，独立审阅要求先完成 F1 阶段 A 返修；返修与审阅依据见页首。PRR-067 协议、PRR-068 C 方案图标和 ULMO 确定性装配方向保持不变；历史候选与证据继续只读保留。
