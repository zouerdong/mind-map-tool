...
### 追加：整理/还原双向开关（`be1e246`，同日闭环）

负责人实机反馈：整理是单向命令——整理后无法一键回到凌乱态，且整理后动过一个节点再撤销只能回到整理态、最初布局被埋。修复：整理变为双向开关（浮动按钮与 ⇧⌘L 同一 dispatcher），再触发即把布局拨回整理前快照；快照 session 级（不入文档），还原本质是正向可撤销的 MoveNodes（中间手动编辑被覆盖但可 ⌘Z 找回，历史零丢失）；文档替换时快照自动失效退化为正常整理。按钮文案随态切换（整理 ⇧⌘L ↔ 还原布局 ⇧⌘L）。旧"幂等 no-op"语义测试按新契约改写（2 处）。原生验证（`verify-toggle-*.png` + 相对坐标证明）：整理→手动移动节点→一次还原，三节点相对坐标与整理前逐点一致。新候选 DMG sha256 `9a85fe7c8b590b9a…` 已安装。
