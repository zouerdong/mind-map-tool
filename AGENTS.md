...
2026-09-21 当前派发更新：`RELEASED_V1.0.0 / PUBLIC_RELEASE_OUT / POST_RELEASE_OPEN`。**v1.0.0 首个公开版本已发布**：https://github.com/zouerdong/mind-map-tool/releases/tag/v1.0.0 （负责人 G-FINAL 原话批准「批准批准，你赶紧的吧」）。发布路径：版本 bump 1.0.0 → entry JS 预算 ADR 0021 重基线 500K→550K（成分证据：react-dom+xyflow+一方代码，无误打包）→ G2 approvedScope 版本对齐 1.0.0（register 附审计修订）→ 候选重打（DMG `fd21164e…`，.app `28facab7…` 跨轮逐字节可复现）→ 完整 v3 证据链（install-gate 受控安装卸载 PASS、原生性能全预算内——冷启动 p95 346ms/RSS 103.8MB/画布帧 19ms、LaunchServices 实测、unsigned adhoc 确认、平台报告装订）→ readiness manifest 驱动 `pnpm quality -- --release-evidence` **20/20 全绿** → tag `v1.0.0`（= 候选源 `29111de`）+ GitHub Release 三资产 + latest。Windows exe 由 CI run 35594973589 产出（`e65f69ec…`），原生自动化证据链维持 DEFERRED。未签名披露已写入 release notes（Gatekeeper/SmartScreen 绕行指引）。下一步：用户/同事反馈收口 → post-release 清理（.tmp 各 attempt 目录，先 grep 禁区引用）→ 下一版本规划。过程记录：unit 门在质量总入口中出现过一次 flaky FAIL（独立重跑与复跑均 824 全过，未定位，低概率）；assets 门越线是 v0.3.x 三批未跑全门暴露的存量问题。

2026-09-20 历史批次（v0.3.2 内测）：`BETA_V032_TAGGED / DUAL_PLATFORM_BETA_OUT / G-FINAL_NOT_REQUESTED`。**v0.3.2 已封板**：负责人四轮 dogfood 通过（「达到预期」），main push 至 `fe3f165`、tag `v0.3.2` 已推远端。双平台内测包在 `.tmp/release-candidate/beta-v0.3.2/`：macOS DMG（sha256 `7e3ad06f…`，含 MCP v3 枚举修复的本地重打件）+ Windows exe（CI run 35500271797，sha256 `0f8a4ea7…` 与 CI 记录一致）+ `agent-mcp-integration.md` v3。本轮批次内容：ADR 0019 v1.1.0→v1.3.0（默认发散/根居中/方向持久化 → 整理后取景与零交叉 rank → 端口序与连线预览主题色 → 左支进线侧右对齐/maxZoom 1 封顶/还原拨回视口）+ 画布禁选治蓝闪 + MCP direction 枚举补 balanced。下一步：同事内测双平台 → 反馈收口 → 候选冻结 + 窄门重验 → GitHub Release（签名/公证/上传/公开发布仍逐项授权）。v0.3.2 R2 dogfood 第三批收口（ADR 0019 v1.2.0 续）：① **端口槽位序改为对端锚沿轴坐标升序**（R2 只修了转折 rank，端口仍按边文档序——真实文档创建序≠高度序时中间槽位被占，麻花复现；负责人实机指出「越高的目标出发点逐一往上排」）。端口序+转折序同向配合，扇出零交叉与输入顺序完全解耦（乱序不变性测试锁定）；② **连线拖拽预览改主题主墨色**（RF 默认连接线冷灰蓝 #b1b1b7 在暖色画布上观感为「蓝闪」）。dense-300-450 等 golden 再 REGEN（原因：端口序变更）。验证：test:unit 823、export golden 18/18、edge-geometry 35/35、typecheck/lint/format 全绿。v0.3.2 R1 dogfood 第二批观感反馈收口（ADR 0019 v1.2.0）：① **整理完成后自适应取景**（fitView padding 0.2 duration 400，整理动画 onComplete 触发，治「内容聚左上角、缩放不匹配」）；② **同缝布线 rank 规则改为垂直行程升序**——v1.1.0 根垂直居中暴露 VRA-040 潜伏缺陷：上方组长行程垂直段必穿短程边进入横段（实机复现 4 处「麻花」交叉），新规则扇出默认零交叉；纯横向/纵向行程序与目标锚序一致，行为不变。dense-300-450 / visual-style-v2 / balanced-fanout 三例 golden REGEN 重基线（原因：本规则变更）。验证：test:unit 823、export golden 18/18、edge-geometry 35/35（新增零交叉锁定测试）、typecheck/lint/format 全绿。下一步：重打 macOS 包 → 负责人 dogfood（整理后自动取景 + 扇出线缆观感）→ 通过后 tag v0.3.2 + CI 双平台 → 同事内测。
