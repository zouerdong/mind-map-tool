...
**优先级：P0**  
**状态：LOCKED**  
**解锁条件：MRT-003V 原生证据通过；ADR 0008 获项目负责人批准**  
**对应问题：CR-004、CR-005**  
**后续：MRT-005**
...
- macOS/Unix 首版增加稳定 metadata identity（device + inode 或等价封装），并保留 canonical path 作为展示/回退。
- identity 的计算和比较只在 host。
- 文件 loading 时立即 reservation，防止并发重复开窗。
- Save As 成功后由 host commit 路径同步更新该窗口 identity。
...
- 阅读 ADR 0008。
- 项目负责人批准后更新 Status/版本与 decision register；未批准不得写生产实现。
...
### MRT-004B：Host registry 与 file identity
...
### MRT-004D：native source 与可靠 ack
