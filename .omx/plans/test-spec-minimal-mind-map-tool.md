...
> 状态：规划基线。工具名为建议；技术栈 ADR 接受后，根 `README.md`/`AGENTS.md` 必须把命令固定为唯一可复制版本。

## 1. 质量门槛

发布候选必须同时满足：core 单元测试、模块集成测试、桌面 E2E、macOS/Windows 人工矩阵、性能预算、三格式导出 golden、安装/文件关联验证。任何一项缺证据都不是“已完成”。
